...
def extract_function(text: str, name: str) -> str:
    """Extract a static NV_STATUS function definition by name."""
    ...
    return_types = list(
        re.finditer(r"(?m)^(?:static\s+)?NV_STATUS\s*$", text[: name_match.start()])
    )
    if not return_types:
...
def validate_source_shape(text: str, pure: str, wrapper: str) -> None:
    ...
    rpc = init_rm.find("NV_RM_RPC_GET_GSP_STATIC_INFO")
    rpc_check = init_rm.find("if (status != NV_OK)", rpc)
    call = init_rm.find(f"status = {LAYOUT_VALIDATOR}", rpc_check)
    call_check = init_rm.find("if (status != NV_OK)", call)
    call_goto = init_rm.find("goto done", call_check)
    trace = init_rm.find("kgspInitGspTraceCrashBuffer", call_goto)
    if not (0 <= rpc < rpc_check < call < call_check < call_goto < trace):
        die(
            "layout validation must be checked after GET_GSP_STATIC_INFO and "
...
#define SEC2_POSTBL_TIMING_MAX_NATIVE_NONPUBLIC_BYTES 0x0000000080000000ULL
#define SEC2_POSTBL_TIMING_BUILD_FINGERPRINT "cmpunlocker-safety-v3"
#define GSP_FW_WPR_META_MAGIC 0x475350574d455441ULL
...
{
    NV2080_CTRL_CMD_FB_GET_FB_REGION_INFO_PARAMS before =
        pContext->kgsp.gspStaticInfo.fbRegionInfoParams;
    NV_STATUS status;
    NvBool succeeded;

    /* validContext() returns by value, so repair its two interior pointers. */
    pContext->kgsp.pWprMeta = &pContext->meta;
    pContext->kgsp.pWprMetaDescriptor = &pContext->descriptor;
    ...
    if (memcmp(&before, &pContext->kgsp.gspStaticInfo.fbRegionInfoParams,
               sizeof(before)) != 0)
    {
        fprintf(stderr, "%s: wrapper mutated the native region table\n", name);
        failures++;
    }
}
...
int main(void)
{
    ...
    addRegion(&params, 14ULL << 32, RSVD_64G, 0, NV_FALSE);
    addTopCarveout(&params, RSVD_64G, FB_64G);
    expectTable("all-16-slots-valid-noop", &params, FB_64G, FB_64G,
                RSVD_64G, MIN_PUBLIC_64G, NV_TRUE,
                RSVD_64G, PROTECTED_BYTES);
...
def run_tests(source: Path) -> None:
    ...
    validate_source_shape(text, pure, wrapper)
